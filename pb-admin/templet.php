...
if (isset($_POST['save']) && !empty($_POST['data']['templet']['title'])) {
    ...
    if (!empty($_POST['data']['site_style'])) {
        ...
        $setting->replace(['site_style' => $the_style]);
        $site_theme_styles = $templet_controller->getStyle();
        $setting->replace(['site_theme_styles' => serialize($site_theme_styles)]);
...
if (isset($_GET['do'])) {
    ...
    if ($do == 'install' && !empty($_GET['entry'])) {
        ...
        $type = trim($_GET['type']);
        // check if exist
        $exists = $pdb->GetRow("SELECT * FROM {$tb_prefix}templets WHERE name='".$entry."' and type='".$type."'");
        if (!empty($exists)) {
            flash();
        }
    ...
    if ($do == 'setup' && !empty($_GET['name']) && ($pdb->GetRow("SELECT * FROM {$tb_prefix}templets WHERE id=".$id))) {
        ...
        $setting->replace(['site_theme' => $the_theme]);
        $result = $cache->writeCache('setting', 'setting');
        if ($result) {
...
foreach ($result as $key => $val) {
    if ($val['type'] == 'user') {
        $val['directory'] = 'templates/'.$val['directory'];
    }
