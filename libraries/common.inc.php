...
$_GET && pb_filter_xss($_GET);
$_POST && pb_filter_xss($_POST);
$_COOKIE && pb_filter_xss($_COOKIE);
if ($_SERVER['REQUEST_URI']) {
    //XSS
    $_GET = pb_htmlspecialchar($_GET);
    $_POST = pb_htmlspecialchar($_POST);
    $_COOKIE = pb_htmlspecialchar($_COOKIE);
}
if (!$admin_runquery) {
    pb_hack_check();
...
if (isset($_PB_CACHE['setting']['time_offset'])) {
    ...
    $time_now = ['time' => gmdate("{$date_format} H:i", $time_stamp + 3600 * $time_offset),
